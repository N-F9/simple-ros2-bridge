...
class ArduinoBridge(Node):
    def __init__(self):
        ...
        self.create_timer(timer_period_sec=1, callback=self.read_from_arduino)
        ...
        self.publisher = self.create_publisher(
            msg_type=String, topic="simple_arduino_bridge_publisher", qos_profile=10
        )

        # Create a timer to publish a toggle message every 10 seconds
        self.timer = self.create_timer(timer_period_sec=10, callback=self.toggle_led)
    ...
    def read_from_arduino(self):
        ...
        while self.serial.in_waiting > 0:
            ...
            self.get_logger().info(
                f'Raw data received from Arduino: {response.decode("utf-8")}'
            )
